...
for target_mod in target_mod_lst:
	positive_probs = get_prob_lst(f"IVT-m1-m2.sample-${target_mod}.taret-${target_mod}.pred.tsv")
	unmodified_base = target_mod[-1]
	for negative_mod in sample_mod_lst:
		if negative_mod == target_mod:
			continue
		negative_probs = get_prob_lst(f"IVT-m1-m2.sample-${negative_mod}.taret-${target_mod}.pred.tsv")
		...
		plt.ylim(0.45,1.05)
		plt.savefig(f"IVT-m1-m2.sample-${negative_mod}.taret-${target_mod}.prc.png",dpi=300)
		plt.close()
