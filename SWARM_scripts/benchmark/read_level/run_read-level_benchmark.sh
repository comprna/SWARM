...
SCRIPT=../../SWARM_scripts/SWARM_read_level.py ;
...
for target_mod in "m6A" "m5C" "pU"; do
        for sample_mod in "m6A" "m5C" 'pU' "NM"; do
                ...
                OUT=IVT-m1-m2.sample-${sample_mod}.taret-${target_mod}.pred.tsv ;
                singularity exec --nv $SIF python3 $SCRIPT -m $target_mod --sam $SAM --fasta $FASTA --raw $BLOW5 -o $OUT ;
        done ;

done

singularity exec --nv $SIF python3 plot_read_level_benchmark.py