...
if __name__ == "__main__":
    ...
    for data_set in ["training", "validation", "testing"]:
        ...
        positive_labels = [POSITIVE_LABEL for _ in range(len(positive_data[0]))]

        negative_data = load_all_data("{}_{}_data.p".format(INPUT_NEGATIVE, data_set))
        negative_labels = [NEGATIVE_LABEL for _ in range(len(negative_data[0]))]
        print("LENS",len(positive_data), len(positive_labels),len(negative_data),len(negative_labels))
        #data = np.vstack((positive_data, negative_data))
        #labels = np.vstack((positive_labels, negative_labels))
        data = np.concatenate((positive_data, negative_data), axis=1)
        data=np.squeeze(data, axis=0).reshape(-1, 100)
