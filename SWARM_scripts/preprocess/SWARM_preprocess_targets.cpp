...
unordered_map<string, unordered_set<int>> get_target_umap(const string& filename) {
...
int main(int argc, char *argv[]) {
    ...
    unordered_map<string, unordered_set<int>> targets_map = get_target_umap(targets_path);
                        ...
                        if (consecutive==5){ // If first 9-mer built
                            ...
                            if (q_count > 0){
                                if (refBases[4] == INPUT_BASE){
                                    if (target_pos_set.count(refPos)){
                                        float out_q[9];
